...
export function SpotlightMyContent(): JSX.Element {
  ...
  const loadPage = (pageNum: number, append = false) => {
    ...
    api
      ...
      }>(API_ENDPOINTS.SPOTLIGHT.MY, { page: pageNum, limit: 20 })
      .then((res: { data?: VideoItem[]; pagination?: typeof pagination }) => {
        const data = res.data || [];
        setVideos((prev) => (append ? [...prev, ...data] : data));
        setPagination(
          res.pagination || { page: 1, totalPages: 0, total: 0 },
        );
      })
    ...
    return (
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-8 text-center dark:border-amber-800 dark:bg-amber-950/30">
        <p className="font-medium text-amber-800 dark:text-amber-200">
          Bạn cần đăng nhập để xem video đã đăng
        </p>
        <Link
          href={`/login?redirect=/spotlight/my`}
  ...
  if (!videos.length) {
    return (
      <div className="py-20 text-center text-muted-foreground">
        <p className="text-lg">Bạn chưa đăng video nào</p>
        <Link
          href="/spotlight/create"
          className="mt-4 inline-block rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 px-5 py-2.5 font-semibold text-white transition hover:opacity-90"
        >
          + Đăng video ngay
        </Link>
      </div>
  ...
  return (
    <div>
      <div className="mb-6 flex justify-end">
        <Link
          href="/spotlight/create"
          className="inline-flex items-center rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-90"
        >
            ...
            <Link href={`/spotlight/${v.id}`} className="block">
              <div className="relative aspect-video overflow-hidden bg-muted">
                ...
                {v.status && v.status !== 'PUBLISHED' && (
                  <span className="absolute right-2 top-2 rounded bg-amber-500/90 px-2 py-0.5 text-xs font-medium text-white">
                    {v.status}
                  </span>
                )}
              </div>
