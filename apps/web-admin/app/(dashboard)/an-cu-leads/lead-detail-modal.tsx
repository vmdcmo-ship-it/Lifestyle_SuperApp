...
const STATUS_LABELS: Record<LeadStatus, string> = {
  PENDING: 'Chờ xử lý', CONTACTED: 'Đã liên hệ', DONE: 'Hoàn thành',
};
...
export function AnCuLeadDetailModal({ leadId, open, onClose, onStatusChange }: Props): JSX.Element {
  const [lead, setLead] = useState<AnCuLead | null>(null);
  ...
  const handleStatusChange = async (newStatus: LeadStatus) => {
    ...
    setUpdating(true);
    try {
      await anCuService.updateStatus(leadId, newStatus);
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Chi tiết lead – An Cư Lạc Nghiệp</DialogTitle>
        </DialogHeader>
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-2">
              ...
              <Select value={lead.status} onValueChange={(v) => handleStatusChange(v as LeadStatus)} disabled={updating}>
            ...
            <div className="grid gap-2 border-t pt-4">
              <p className="text-xs text-muted-foreground">
                Nguồn: <span className="font-medium text-foreground">{lead.source}</span> • Tạo lúc: {formatDate(lead.createdAt)}
              </p>
            </div>
          </div>
        ) : null}
