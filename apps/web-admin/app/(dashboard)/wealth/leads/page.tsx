...
export default function WealthLeadsPage(): JSX.Element {
  const [items, setItems] = useState<WealthLead[]>([]);
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      const res = await wealthService.listLeads(params);
      setItems(res.data ?? []);
      setPagination(
  ...
  const handleStatusChange = async (lead: WealthLead, newStatus: LeadStatus) => {
    setUpdatingId(lead.id);
    try {
      await wealthService.updateStatus(lead.id, newStatus);
      setItems((prev) =>
  ...
  const openDetail = (lead: WealthLead) => {
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl font-bold">KODO Wealth – Leads tư vấn</h1>
